...
В интерфейс добавлена полноценная русская локализация: кнопки, метки и описания переключаются между английским и русским языками на основе выбранного языка пользователя.
Чтобы сменить язык, откройте левую панель (`Worldview Left Panel`), нажмите на кнопку с текущим языком (🇷🇺/🇺🇸) в заголовке и выберите нужную локаль — все поля мгновенно адаптируются.
